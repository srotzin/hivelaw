/**
 * Jurisdiction Registry — PostgreSQL-backed with in-memory fallback.
 * Pre-seeded with 12 jurisdictions via UPSERT.
 */

import pool, { isDbAvailable } from './db.js';

// In-memory fallback
const memJurisdictions = new Map();
...
// Always seed in-memory as fallback
for (const j of JURISDICTION_DATA) {
  memJurisdictions.set(j.code, j);
}
...
export async function seedJurisdictions() {
  ...
  try {
    for (const j of JURISDICTION_DATA) {
      await pool.query(`
        INSERT INTO hivelaw.jurisdictions (code, name, type, parent, regulations, supported, compliance_requirements)
      ...
      `, [
        ...
        j.type,
        j.parent,
        JSON.stringify({ ...j.regulations, governing_law: j.governing_law, hallucination_default: j.hallucination_default }),
        j.supported,
        j.compliance_requirements,
      ]);
    }
  } catch (err) {
    console.error('[jurisdiction-registry] Seed failed:', err.message);
  }
}

export function getJurisdiction(code) {
  // Always use in-memory for jurisdiction data since it includes governing_law/hallucination_default
  // which the DB stores inside regulations JSONB
  return memJurisdictions.get(code) || null;
}

export function listJurisdictions() {
  return Array.from(memJurisdictions.values()).map(j => ({
    code: j.code,
    name: j.name,
    type: j.type,
    parent: j.parent,
...
export function checkCompliance(code, contractType = 'service_agreement') {
  const j = memJurisdictions.get(code);
