...
const SAGA_DEFINITIONS = {
  contract: {
    ...
    compensations: {
      mind_record: async (state) => {
        console.log(`[Saga] Compensating mind_record: archiving memory for contract ${state.contract_id}`);
      },
      trust_verify_parties: async (state) => {
        console.log(`[Saga] Compensating trust_verify_parties: logging verification failure for contract ${state.contract_id}`);
      },
      law_create_contract: async (state) => {
        console.log(`[Saga] Compensating law_create_contract: voiding contract ${state.contract_id}`);
      },
    },
  },
};
...
export async function createSaga(type, initialState = {}) {
  ...
  if (!pool) {
    console.log(`[Saga] No database — saga ${sagaId} (${type}) logged but not persisted`);
    return sagaId;
  }
  await pool.query(
...
export function startSagaWorker() {
  if (!pool) {
    console.log('  [Saga] No database — worker disabled');
    return;
  }
  setInterval(async () => {
    try {
